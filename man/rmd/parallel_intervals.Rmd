...
To use parallelsim, the [future::plan()] function can be invoked to create a parallel backend. For example, let's make an initial workflow: 
...
int_conformal_infer(lm_fit, new_dat, train_data = train_dat)
```
